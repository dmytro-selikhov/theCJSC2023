...
const rendorError = function(msg){
...
// 	.catch(err => {
// 		console.error(`${err} :(`);
// 		rendorError(`Somerhing went wrong, ${err.message}. Try again!`);
// 	})
// 	.finally(() => {
// 		countriesContainer.style.opacity = 1;
// 	});
...
const getCountryData = function(country) {
	...
	.then(data => {
		renderCountry(data[0]);
		const neighbour = data[0].borders[0];
			// This sheet doesn't work(I mean the Error)
...
const whereAmI = function(lat, lng){
	...
	.then(res => {
		if (!res.ok) throw new Error(`Problem with service ${res.status}`);
		return res.json()})
	.then(data => {
		// console.log(data);
...
const lotteryPromise =  new Promise(function(resolve, reject){
	console.log('Lottery draw is happaning!');
	setTimeout(function(){
		if (Math.random() >= 0.5){
			resolve('You WIN money!:D')
		} else {
...
// .catch(err => console.error(`2: ${err.message} I don't smiles here`))
...
(async function(){
	try {
		...
		console.error(`2: ${err.message} I don't smiles here`);
	}
